export const getAllRecensioni = () => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.59:5220/api/Recensioni/recensioni", {
                headers: {
...
export const getMiaRecensione = () => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.59:5220/api/Recensioni/recensione", {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("jwtToken"),
                    "Content-Type": "application/json",
                },
...
export const postRecensione = (testo, voto) => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.59:5220/api/Recensioni", {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("jwtToken"),
                    "Content-Type": "application/json",
                },
    ...
    return async (dispatch) => {
        try {
            const response = await fetch(
                "http://192.168.1.59:5220/api/Recensioni",
                {
                    method: "PUT",
                    body: JSON.stringify({
                        testo: testo,
