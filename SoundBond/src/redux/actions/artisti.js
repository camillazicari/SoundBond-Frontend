export const getArtisti = () => {
  return async (dispatch) => {
    try {
      const response = await fetch("http://192.168.1.12:5220/api/Artisti", {
        headers: {
  ...
  return async (dispatch) => {
    try {
      const response = await fetch(
        "http://192.168.1.12:5220/api/Artisti/artista?nome=" +
...
export const deleteArtista = (nome) => {
  return async (dispatch) => {
    try {
      const response = await fetch(
        "http://192.168.1.12:5220/api/Artisti?nome=" + encodeURIComponent(nome),
        {
          method: "DELETE",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("jwtToken"),
