export const getGeneri = () => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.60:5220/api/Generi", {
                headers: {
    ...
    return async (dispatch) => {
        try {
            const response = await fetch(
                "http://192.168.1.60:5220/api/Generi/genere?nome=" + encodeURIComponent(nome),
                {
                    method: "PUT",
                    body: JSON.stringify({
                        nome: nome,
...
export const deleteGenere = (nome) => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.60:5220/api/Generi/genere?nome=" + encodeURIComponent(nome), {
                method: "DELETE",
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("jwtToken"),
                },
