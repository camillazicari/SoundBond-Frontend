export const getRichiesteInviate = () => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.60:5220/api/Richiesta/inviate", {
                headers: {
...
export const getRichiesteRicevute = () => {
    return async (dispatch) => {
        try {
            const response = await fetch("http://192.168.1.60:5220/api/Richiesta/ricevute", {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("jwtToken"),
                    "Content-Type": "application/json",
                },
...
export const postRichiesta = (id) => {
    return async (dispatch, getState) => {
        try {
            ...
            const response = await fetch("http://192.168.1.60:5220/api/Richiesta", {
                headers: {
                    Authorization: "Bearer " + localStorage.getItem("jwtToken"),
                    "Content-Type": "application/json",
                },
    ...
    return async (dispatch) => {
        try {
            ...
            const response = await fetch(
                "http://192.168.1.60:5220/api/Richiesta?id=" + id,
                {
                    method: "DELETE",
                    headers: {
                        Authorization: "Bearer " + localStorage.getItem("jwtToken"),
